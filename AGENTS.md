...
- TestFlight deployments must build a commit already merged into `main`. Trigger one by commenting exactly `/testflight` on the merged PR (repository owner/member/collaborator or `cursor[bot]`), or use the workflow's manual `main`-branch dispatch as a fallback.
