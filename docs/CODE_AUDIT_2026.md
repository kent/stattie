...
| High | Player, game, membership, and profile flows silently ignored saves, sometimes dismissing on failure. | Added a shared persistence controller and error alert. Failed saves roll back; editors dismiss only after a successful commit. Existing tracking errors use the same presenter. |
...
- CloudKit status is based on container events, not a per-record reconciliation proof. Multi-device conflict handling, duplicated singleton preference rows, and account switching warrant signed-device integration tests. The unsigned hosted suite cannot validate Apple's live synchronization service.
- Preferences and achievements still maintain a local defaults mirror. Their merge semantics and side-effecting reads are candidates for a dedicated observable repository layer.
